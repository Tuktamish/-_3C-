...
public class FileOperations
{
    ...
    public static int[,] CreateTwoDimensionalArray(int[] oneDimensionalArray)
    {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            for (int i = 0; i < n - 1; i++)
            {
                // Заполняем строки случайными числами от 1 до (sum - currentSum)
                resultMatrix[i, j] = new Random().Next(1, sum - currentSum + 1);
                currentSum += resultMatrix[i, j];
            }

            // Последний элемент строки
            resultMatrix[n - 1, j] = sum - currentSum; // Убедимся, что сумма равна
        }
    ...
    public static void FindMaxMinProduct(string filePath)
    {
        ...
        int max = numbers.Max();
        int min = numbers.Min();
        Console.WriteLine($"Произведение max и min: {max * min}");
    }
